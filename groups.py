...
def schoenflies_name_from_hm_symbol(hm_symbol):
    return POINTGROUP_MAPPING[[_[1] for _ in POINTGROUP_MAPPING].index(hm_symbol)][2]
...
def rotation(axis, angle):
    ...
    q = [np.cos(angle / 2)]
    for a in axis:
        q.append(np.sin(angle / 2.0) * a / norm)
    q = Quaternion(q)
...
def rotoreflection(axis, angle):
    ...
    return -rotation(axis, angle + np.pi)  # pylint: disable=invalid-unary-operand-type


# Shorhand notation for complex factors appearing in the character table
w = np.exp(2j * np.pi / 3.0)
w2 = np.exp(-2j * np.pi / 3.0)
...
pointgroup_dict = {
    ...
    "D2": {
        "classes": {
            0: [np.identity(3)],  # E
            1: [rotation([0, 0, 1], np.pi)],  # C2z
            2: [rotation([0, 1, 0], np.pi)],  # C2y
            3: [rotation([1, 0, 0], np.pi)],  # C2x
        },
        "character_table": {
            "A": [1, 1, 1, 1],
            "B1": [1, 1, -1, -1],
    ...
    "C4": {
        "classes": {
            0: [np.identity(3)],  # E
            1: [rotation([0, 0, 1], np.pi)],  # C2
            2: [rotation([0, 0, 1], np.pi / 2.0)],  # C4+
            3: [rotation([0, 0, 1], -np.pi / 2.0)],  # C4-
        },
        "character_table": {
            "A": [1, 1, 1, 1],
            "B": [1, 1, -1, -1],
    ...
    "S4": {
        "classes": {
            0: [np.identity(3)],  # E
            1: [rotation([0, 0, 1], np.pi)],  # C2
            2: [rotoreflection([0, 0, 1], np.pi / 2.0)],  # S4+
            3: [rotoreflection([0, 0, 1], -np.pi / 2.0)],  # S4-
        },
        "character_table": {
            "A": [1, 1, 1, 1],
            "B": [1, 1, -1, -1],
    ...
    "C4h": {
        "classes": {
            ...
            4: [-np.identity(3)],  # I
            5: [rotoreflection([0, 0, 1], 0.0)],  # sigma_h
            6: [rotoreflection([0, 0, 1], -np.pi / 2.0)],  # S4-
            7: [rotoreflection([0, 0, 1], np.pi / 2.0)],  # S4+
        },
        "character_table": {
            "Ag": [1, 1, 1, 1, 1, 1, 1, 1],
            "Bg": [1, 1, -1, -1, 1, 1, -1, -1],
    ...
    "D4": {
        "classes": {
            ...
            1: [rotation([0, 0, 1], np.pi)],  # C2
            2: [
                rotation([0, 0, 1], np.pi / 2.0),
                rotation([0, 0, 1], -np.pi / 2.0),
            ],  # C4
            3: [rotation([1, 0, 0], np.pi), rotation([0, 1, 0], np.pi)],  # C2h
            4: [rotation([1, 1, 0], np.pi), rotation([1, -1, 0], np.pi)],  # C2h'
        },
        "character_table": {
            "A1": [1, 1, 1, 1, 1],
            "A2": [1, 1, 1, -1, -1],
    ...
    "D2d": {
        "classes": {
            ...
            1: [rotation([0, 0, 1], np.pi)],  # C2
            2: [
                rotoreflection([0, 0, 1], np.pi / 2.0),
                rotoreflection([0, 0, 1], -np.pi / 2.0),
            ],  # S4
            3: [rotation([1, 0, 0], np.pi), rotation([0, 1, 0], np.pi)],  # C2h
            4: [
    ...
    "C3": {
        "classes": {
            0: [np.identity(3)],  # E
            1: [rotation([0, 0, 1], 2 * np.pi / 3.0)],  # C3+
            2: [rotation([0, 0, 1], -2 * np.pi / 3.0)],  # C3-
        },
        "character_table": {
            "A": [1, 1, 1],
            "1E": [1, w2, w],
    ...
    "C3i": {
        "classes": {
            ...
            2: [rotation([0, 0, 1], -2 * np.pi / 3.0)],  # C3-
            3: [-np.identity(3)],  # I
            4: [rotoreflection([0, 0, 1], -np.pi / 3.0)],  # S6-
            5: [rotoreflection([0, 0, 1], np.pi / 3.0)],  # S6+
        },
        "character_table": {
            "Ag": [1, 1, 1, 1, 1, 1],
            "1Eg": [1, w2, w, 1, w2, w],
    ...
    "D3": {
        "classes": {
            0: [np.identity(3)],  # E
            1: [
                rotation([0, 0, 1], 2 * np.pi / 3.0),
                rotation([0, 0, 1], -2 * np.pi / 3.0),
            ],  # C3
            2: [
                rotation([1, 0, 0], np.pi),
                rotation([-1, np.sqrt(3.0), 0], np.pi),
                rotation([-1, -np.sqrt(3.0), 0], np.pi),
            ],  # C2
        },
        "character_table": {
    ...
    "C3v": {
        "classes": {
            ...
            2: [
                rotoreflection([1, 0, 0], 0.0),
                rotoreflection([-1, np.sqrt(3.0), 0], 0.0),
                rotoreflection([-1, -np.sqrt(3.0), 0], 0.0),
            ],  # sigma_v
        },
        "character_table": {
    ...
    "D3d": {
        "classes": {
            ...
            3: [-np.identity(3)],  # I
            4: [
                rotoreflection([0, 0, 1], -np.pi / 3.0),
                rotoreflection([0, 0, 1], np.pi / 3.0),
            ],  # S3
            5: [
                rotoreflection([1, 0, 0], 0.0),
    ...
    "C6": {
        "classes": {
            0: [np.identity(3)],  # E
            1: [rotation([0, 0, 1], np.pi / 3.0)],  # C6+
            2: [rotation([0, 0, 1], 2 * np.pi / 3.0)],  # C3+
            3: [rotation([0, 0, 1], np.pi)],  # C2
            4: [rotation([0, 0, 1], -2 * np.pi / 3.0)],  # C3-
            5: [rotation([0, 0, 1], -np.pi / 3.0)],  # C6-
        },
        "character_table": {
            "A": [1, 1, 1, 1, 1, 1],
            "B": [1, -1, 1, -1, 1, -1],
    ...
    "C3h": {
        "classes": {
            ...
            4: [rotoreflection([0, 0, 1], -2 * np.pi / 3.0)],  # S3-
            5: [rotoreflection([0, 0, 1], 2 * np.pi / 3.0)],  # S3+
        },
        "character_table": {
            "A'": [1, 1, 1, 1, 1, 1],
            "A''": [1, 1, 1, -1, -1, -1],
    ...
    "C6h": {
        "classes": {
            ...
            5: [rotation([0, 0, 1], -np.pi / 3.0)],  # C6-
            6: [-np.identity(3)],  # E
            7: [rotoreflection([0, 0, 1], -2 * np.pi / 3.0)],  # S3-
            8: [rotoreflection([0, 0, 1], -np.pi / 3.0)],  # S6-
            9: [rotoreflection([0, 0, 1], 0.0)],  # sigma_h
            10: [rotoreflection([0, 0, 1], np.pi / 3.0)],  # S6+
            11: [rotoreflection([0, 0, 1], 2 * np.pi / 3.0)],  # S3+
        },
        "character_table": {
    ...
    "D6": {
        "classes": {
            ...
            5: [
                ...
                rotation([np.sqrt(3.0), 1, 0], np.pi),
                rotation([np.sqrt(3.0), -1, 0], np.pi),
            ],  # C2h'
        },
        "character_table": {
    ...
    "C6v": {
        "classes": {
            ...
            5: [
                rotoreflection([0, 1, 0], 0.0),
                rotoreflection([np.sqrt(3.0), 1, 0], 0.0),
                rotoreflection([np.sqrt(3.0), -1, 0], 0.0),
            ],  # sigma_v'
        },
        "character_table": {
    ...
    "D3h": {
        "classes": {
            ...
            3: [
                rotoreflection([0, 0, 1], 2 * np.pi / 3.0),
                rotoreflection([0, 0, 1], -2 * np.pi / 3.0),
            ],  # S3
            4: [
                rotation([1, 0, 0], np.pi),
...
class Pointgroup:
    ...
    def _check_orthogonality_irreps(self):
        ...
        test = np.identity(num_irreps) * (self.get_order() + 0j)
        for c, sym_ops in self.classes.items():
            for i, chars1 in enumerate(self.character_table.values()):
                for j, chars2 in enumerate(self.character_table.values()):
                    test[i, j] -= chars1[c] * np.conj(chars2[c]) * len(sym_ops)
        return np.sum(abs(test)) < 1.0e-5
        ...
        for (
            ...
            for i in range(num_classes):
                for j in range(num_classes):
                    test[i, j] -= characters[i] * np.conj(characters[j])
        return np.sum(abs(test)) < 1.0e-5
...
def get_spacegroup_and_pointgroup(structure):
